...
ggplot(subset(reshapedUS, variable == c("Murder", "Assault")), aes(x=value, colour = variable, fill = variable)) +
...
\footnotesize
```{r, fig.height=5.8, cache=TRUE}
ggplot(subset(reshapedUS, variable == c("Murder", "Assault")), aes(x=value, fill = variable)) +
  geom_histogram(bins=50, position="identity", colour = "black") +
...
ggplot () + 
  geom_bar(data=subset(reshapedUS, variable == "Murder"), 
           aes(x= state, y= value), stat="identity") +
  geom_line(data = subset(reshapedUS, variable == "UrbanPop"), aes(x=state, y=value, group=1)) +
  theme(axis.text.x = element_text(angle = 45))
```
...
barchart <- ggplot () + 
  geom_bar(data=subset(reshapedUS, variable != "UrbanPop"), 
